...
import { getUpcommingTask } from '../../actions/addTask';
...
const UpcomingPage = () => {
    ...
    const upcommingTask = useSelector((state) => state.addTask.responseUpcommingData);
    ...
    useEffect(() => {
        dispatch(getUpcommingTask(userId));
    }, [upcommingTask])
        ...
        <Container>
                <div className="upcomming-page-container container-fluid">
                    <div className="d-flex align-items-center">
                        <Typography variant="h4">Upcomming Task</Typography>
                        <span className="ms-4 border fw-bold fs-2 ps-3 pe-3">{upcommingTask && upcommingTask.length}</span>
                    </div>
                    <div className="addTask-parent-container border mt-4">
                        <div className="p-4">
                            <div className="mt-3">
                                <div>
                                    {upcommingTask && upcommingTask.length === 0 ? <p>No Upcomming Task</p> :
                                       upcommingTask && upcommingTask.map((val) => (
                                            <div id={val._id} key={val._id} className='task-div border mt-3 d-flex justify-content-between'>
                                                <div className='pb-2'>
                                                    <div className='d-flex align-items-center ms-3 mt-3'>
                                                        <input type='checkbox' onChange={() => console.log("haha")} className='checkbox' />
